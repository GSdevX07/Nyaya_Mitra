...
DEFAULT_SENDER = os.getenv("NOTIFICATIONS_SENDER_EMAIL", "notifications@nyayamitra.gov.in")
...
class SimulatedEmailDriver(BaseEmailDriver):
    ...
    def send_email(
        ...
        sender_addr = sender or DEFAULT_SENDER
        msg_id = f"EMAIL-{uuid.uuid4().hex[:12].upper()}"
        logger.info(f"[EMAIL SIMULATION] To: {to_email} | Sender: {sender_addr} | Subject: '{subject}' | Ref: {msg_id}")
...
class EmailAdapter(BaseChannelAdapter):
    ...
    def send(
        ...
        if not self._validate_email(email):
            logger.warning(
                f"[EMAIL] Cannot deliver notification {record.id}: recipient email missing or invalid format ({email})."
            )
            return ChannelDeliveryResult(
                success=False,
                channel=NotificationChannel.EMAIL,
                error="Recipient email address missing or invalid",
                retryable=False,
            )

        # Dynamic statutory/legal notice text
        if record.payload.get("statutory_reference"):
            legal_footer = f"Statutory compliance under {record.payload['statutory_reference']}."
        elif record.event_type.value == "APPROACHING_CUSTODY_THRESHOLD":
            legal_footer = "Statutory compliance under Section 479 Bharatiya Nagarik Suraksha Sanhita (BNSS), 2023."
        else:
            legal_footer = "Official legal docket alert generated by Nyaya Mitra Judicial Services."
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px;">
            ...
            <p style="font-size: 12px; color: #64748b;">Generated at {record.created_at} UTC. {legal_footer}</p>
        </div>
        """
